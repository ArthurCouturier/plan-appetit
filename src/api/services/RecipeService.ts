...
export default class RecipeService {
      ...
      ingredients: [
        {
          ...
          season: [],
          quantity: { value: 0, unit: UnitEnum.CENTILITER },
        },
      ],
      covers: 0,
      buyPrice: 0,
  ...
  static async fetchRecipesRemotly(): Promise<RecipeInterface[]> {
    ...
    const recipes: RecipeInterface[] = await BackendService.getPersonalRecipes(
      email,
      token
    );
    localStorage.setItem("recipes", JSON.stringify(recipes));
  ...
  static async updateRecipe(recipe: RecipeInterface) {
    const email: string = localStorage.getItem("email") as string;
    const token: string = localStorage.getItem("firebaseIdToken") as string;
    const updateRecipe = await BackendService.updateRecipe(
      email,
      token,
      recipe
    );
  ...
  static importRecipe(
    ...
    const recipes: RecipeInterface[] =
      RecipeService.fetchRecipesLocally() as RecipeInterface[];
    ...
    reader.onload = async (event) => {
      ...
      const newRecipe = await BackendService.importRecipeFromLocalFile(
        jsonData,
        email,
        token
      );
    ...
    if (data instanceof File) {
      ...
      localStorage.setItem("recipes", JSON.stringify(recipes));
      alert(
        `Importation réussie ! Les données ont été enregistrées dans localStorage (clé 'recipes').`
      );
      if (setRecipes) {
        setRecipes(recipes);
      }
